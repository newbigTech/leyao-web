          ...
          <table>
            <tr v-for="(tr,i) in trs">
              <td v-for="(td, i) in tr"
                ...
                :rowspan="td.rowspan"
                :colspan="td.colspan"
                :class="{'active': td.active, 'block': td.block, 'first': td.first}"
                >{{td.id}}</td>
            </tr>
          </table>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSelectingTds(tableArray, td) {
        let _tableArray = []
        for (let i = 0, lena = this.tableArray.length; i < lena; i++) {
          let trs = []
          let lenb = this.tableArray[i].length
          for (let ii = 0; ii < lenb; ii++) {
            let _td = this.tableArray[i][ii]
            if (td.x <= _td.x && td.y <= _td.y ) {
              trs.push(_td)
            }
          }
          if (trs.length > 0) _tableArray.push(trs)
        }
        return _tableArray
      },
      ...
      tdClick(td) {
        ...
        // get all tds no active flag
        let trsPure = this.getSelectingTds(this.trs, td)
        // 标记 block
        trsPure.forEach(trs => {
          trs.forEach(td => {
            if (td.first || td.active && !td.block) {
              doMarkBlockTds(trsPure, td)
              return
            }
          })
        })
        ...
        if (trs22.length > 0) this.trs2 = trs22
      },
