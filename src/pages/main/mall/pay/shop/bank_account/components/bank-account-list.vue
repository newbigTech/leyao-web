...
<script>
  ...
  import { _getShopBankAccountList,
           _setShopDefaultReceiptAccount,
           _deleteShopReceiptAccount,
           _addShopBankAccount } from 'src/vuex/actions/payAction'
  ...
  export default {
    ...
    methods: {
      searchTableList() {
        _getShopBankAccountList({}).then(data => {
      ...
      setDefault(accountId){
        this.$confirm('是否设置默认账户?', '提示', {
          type: 'warning'
           ...
           _setShopDefaultReceiptAccount({"accountId":accountId}).then(data => {
            const { success, result, msg } = data
            if (success) {
              this.searchTableList() ;
      ...
      handleSubmit(){
        this.$refs.bankAccount.validate((valid) => {
          if (valid) {
            ...
            _addShopBankAccount(params).then(data => {
              const { success, result, msg } = data
      ...
      deleteBankAccount(accountId){
        this.$confirm('是否确定删除?', '提示', {
          type: 'warning'
             ...
             _deleteShopReceiptAccount({"receiptAccountIds":accountId}).then(data => {
              const { success, result, msg } = data
              if (success) {
                this.searchTableList() ;
