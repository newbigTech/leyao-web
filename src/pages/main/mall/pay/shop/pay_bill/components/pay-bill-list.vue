...
<script>
  ...
  import { _getShopPayBillListPage } from 'src/vuex/actions/payAction'
  ...
  export default {
    ...
    methods: {
      ...
      searchTableList() {
        ...
        _getShopPayBillListPage(tempParams).then(data => {
      ...
      exportCurrentPage(){
        ...
        tempParams.pageNo = this.page
        tempParams.pageSize = this.pageSize
        location.href='/pay/mallShopExportPayController/exportPayBillList?'+$.param(tempParams) ;
      },
      // 导出所有
      exportAllPage(){
        let tempParams = this.initParams() ;
      ...
      clickDetail(row) {
        this.$router.push({ name: 'financial_shop_detail', params: { billDate: row.billDate }})
      }
    }
  }
</script>
