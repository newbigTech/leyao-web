      ...
      <div class="_table-container">
        <div class="_table-list">
            <el-table :data="financialData" style="width: 100%" border>
                ...
                <el-table-column align="center" property="createTime" label="创建日期" :formatter="dateFormat" ></el-table-column>
                <el-table-column align="center" inline-template label="资金业务类型">
                  <div>
                    ...
                    <span v-if="row.businessType == 'DEPOSIT'">保证金</span>
                  </div>
                </el-table-column>
                ...
                <el-table-column align="center" inline-template label="操作">
                      <div>
                        <el-button
                          size="small"
                          type="danger"
                          @click.native.prevent="onViewOrder(row)"
                          v-if="row.businessType == 'CONSUME' ||
                                row.businessType == 'RECHARGE' ||
                                row.businessType == 'SERVICE_FEE'">详情</el-button>
                        <el-button
                          size="small"
                          type="danger"
                          @click.native.prevent="$emit('forward', 'withdrawDetail', row)"
                          v-if="row.businessType == 'WITHDRAW'">详情</el-button>
                      </div>
                </el-table-column>
            </el-table>
            <!-- 分页 -->
...
<script>
  ...
  import { _getFundRecordPage } from 'src/vuex/actions/payAction'
  ...
  export default {
    ...
    methods: {
      ...
      searchTableList() {
        ...
        _getFundRecordPage(tempParams).then(data => {
      ...
      onViewOrder(item) {
            ...
            let params = {
                orderNo: item.linkNo,
                urlPathName:'financial_supplier_detail',
                r: _.toNumber(_.now())
            }
      ...
      exportCurrentPage(){
        ...
        location.href='/pay/mallExportPayController/exportFundRecordList?'+$.param(tempParams) ;
      },
      // 导出所有
      exportAllPage(){
        let tempParams = this.initParams() ;
