<template>
  <div class="_classify-property-item">
    <card-panel>
      ...
      <div slot="panel-btns">
        <button class="btn btn-sm btn-success" @click="add"  v-show="!sku_checked">新增</button>
      </div>
        ...
        <table class="table table-hover center">
          <tbody>
            ...
            <tr v-for="(r, i) in tableData">
              ...
              <td>{{r.sortIndex}}</td>
              <td width="15%">
                <button class="btn btn-xs btn-warning" @click="edit(r)" v-show="!sku_checked">编辑</button>
                <button class="btn btn-xs btn-danger" @click="remove(r)" v-show="!sku_checked">删除</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogShow">
      <el-form :model="skuItem" :rules="rules" ref="skuItem">
        ...
        <el-form-item label="备注" :label-width="formLabelWidth">
            <el-input type="textarea" v-model="skuItem.comments" style="width:50%" :maxlength="200"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      edit(row){
        ...
        this.skuItem.comments=row.comments
        if (row.itemPic) {
    ...
    created() {
      ...
      this.e.$on('load-sku-item' + this.id, item => {
        ...
        this.sku_checked=item.sku_checked
