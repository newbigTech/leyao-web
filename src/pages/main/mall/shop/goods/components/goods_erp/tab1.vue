            ...
            <el-col :span="12">
                <el-form :model="goodsObj" :rules="rules" ref="goodsObj" label-width="100px" style="min-width: 370px;max-width: 550px;">
                    <el-form-item label="商品主分类" prop="classifyId">
                        ...
                        <el-input v-model="goodsObj.classifyName" :maxlength="20" readonly placeholder="请选择主分类" style="width:50%"></el-input>
                        &nbsp;&nbsp;&nbsp;
                        <el-button type="text" @click.native="onSelectedERPGoods" v-show="!goodsObj.goodsId">选择商品来源</el-button>
                    ...
                    <el-form-item label="商品类型">
                        <span v-if="goodsObj.goodsType===0">单品</span>
                        <span v-if="goodsObj.goodsType===1">规格商品</span>
                        <span v-if="goodsObj.goodsType===2">组合商品</span>
                    </el-form-item>
                    <el-form-item label="库存类型">
                        ...
                        <span v-if="goodsObj.stockType===0">无限制</span>
                        <span v-if="goodsObj.stockType===1||goodsObj.stockType===3">独立库存</span>
                    </el-form-item>
                ...
                <erp-goods-dialog ref="shopERPGoodsComm" @onSuccess="getShopGoodSuccess"></erp-goods-dialog>
            </el-col>
        </el-row>
    </div>
</template>
<script>
import ErpGoodsDialog from './erp-goods-dialog'
...
export default {
    ...
    methods: {
        getShopGoodSuccess(data) {
            ...
            this.goodsObj.erpGoodsId=data.id
            this.goodsObj.goodsType=data.goodsType
            if(data.goodsType===1&&data.stockType===1){
                 this.goodsObj.stockType=3
            }else{
                this.goodsObj.stockType=data.stockType
            }
            this.$store.getters.ev.$emit('onchangeGoodsType', data.goodsType)
            getERPGoodsDetail(params).then(data=>{
                const { success, msg, result } = data
                if (success) {
                    console.log('resultJson',result)
                    this.$store.getters.ev.$emit('onLoadERPData', result)
                    this.goodsObj.goodsSn=result.goodsSn
                    this.goodsObj.goodsName=result.goodsName
                    this.goodsObj.goodsFullName=result.slaveTitle
                    this.goodsObj.goodsDesc=result.goodsDesc
                    this.goodsObj.stockNumber=result.stockNumber||0
                    //this.goodsObj.mprice=result.marketPrice||0
                    //this.goodsObj.price=result.salesPrice||0
                } else {
        ...
        onSelectedERPGoods(){
            this.$refs.shopERPGoodsComm.goodsDataVisible = true
        },
        onGenerator(){
                ...
                if (valid) {
                    ...
                    saveGoodsFormTab1(this.goodsObj).then(data => {
                        ...
                        if (success) {
                            ...
                            if(this.goodsObj.erpGoodsId){
                                if(this.goodsObj.goodsType==1){
                                    this.$store.getters.ev.$emit('onSaveGoodsSku') 
                                }
                                 this.goodsObj.erpGoodsId=undefined
                            }
