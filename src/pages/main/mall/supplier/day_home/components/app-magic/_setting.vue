  ...
  <appSettingBox class="_app-magic-setting">
          ...
          <table class="_magic-table _magic-4x4">
            <tr v-for="(tr, i) in obj.prop.trs">
              <td v-for="(td, ii) in tr"
                ...
                :data-y="td.y"
                :data-id="td.id"
                :class="['col' + td.colspan, 'row' + td.rowspan, td.active ? 'active': '', td.first ? 'block' : '', currentId == td.id && td.active ? 'current' : '']"
                @click="tdClick(td)"
              >
    ...
    <div class="_magic-link-wrapper">
      <ul class="_magic-link">
        <li class="_magic-link-item" v-for="(b, i) in this.blocks" :data-id="b[0][0].id" v-show="currentId == b[0][0].id">
          <div class="_row-flex">
            <label>选择图片: </label>
            <div class="_row-auto">
              <div class="_magic-link-item-img" @click="selectImg(b[0][0])">
                <img :src="b[0][0].img" v-show="b[0][0].img.length > 0">
          ...
          <div class="_row-flex">
            <label>选择链接: </label>
            <el-input placeholder="请选择图片链接" v-model="b[0][0]['link']" size="small" class="_row-auto" style="margin-left: 5px;" :disabled="true"></el-input>
            <el-button size="small" style="margin-left: 5px; margin-right: 5px;" >选择链接</el-button>
          </div>
      ...
      tdClick(td) {
        ...
        if (this.trs2.length == 1 && this.trs2[0].length == 1) {
          ...
          this.blocks.forEach(b => {
            let _b = []
            b.forEach(tr => tr.forEach(td => _b.push(td.id)))
